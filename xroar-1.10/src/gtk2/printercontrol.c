...
struct uigtk2_dialog *gtk2_pc_dialog_new(struct ui_gtk2_interface *uigtk2) {
	struct uigtk2_dialog *dlg = uigtk2_dialog_new(uigtk2, "/uk/org/6809/xroar/gtk2/printercontrol.ui", "pc_window", ui_tag_print_dialog);

	// Join each UI group we're interested in
	ui_messenger_join_group(dlg->msgr_client_id, ui_tag_print_destination, MESSENGER_NOTIFY_DELEGATE(pc_ui_state_notify, uigtk2));
	ui_messenger_join_group(dlg->msgr_client_id, ui_tag_print_file, MESSENGER_NOTIFY_DELEGATE(pc_ui_state_notify, uigtk2));
	ui_messenger_join_group(dlg->msgr_client_id, ui_tag_print_pipe, MESSENGER_NOTIFY_DELEGATE(pc_ui_state_notify, uigtk2));
	ui_messenger_join_group(dlg->msgr_client_id, ui_tag_print_count, MESSENGER_NOTIFY_DELEGATE(pc_ui_state_notify, uigtk2));

	// Connect signals

	uigtk2_signal_connect(uigtk2, "pc_b_file_attach", "clicked", pc_file_attach, uigtk2);
	uigtk2_signal_connect(uigtk2, "pc_e_pipe", "changed", pc_pipe_changed, uigtk2);
	uigtk2_signal_connect(uigtk2, "pc_b_pipe_reset", "clicked", pc_pipe_reset, uigtk2);
	uigtk2_signal_connect(uigtk2, "pc_e_pipe", "activate", pc_pipe_apply, uigtk2);
	uigtk2_signal_connect(uigtk2, "pc_b_pipe_apply", "clicked", pc_pipe_apply, uigtk2);
	uigtk2_signal_connect(uigtk2, "pc_b_flush", "clicked", pc_flush, uigtk2);

	for (unsigned i = 0; i < ARRAY_N_ELEMENTS(rb_destinations); i++) {
		uigtk2_signal_connect(uigtk2, rb_destinations[i].id, "clicked", pc_set_destination, uigtk2);
	}

#ifndef HAVE_POPEN
	// Remove pipe from the UI if not supported.  Not that I can imagine
	// this ever being the case...
	uigtk2_widget_set_sensitive(uigtk2, "pc_rb_pipe", 0);
	uigtk2_widget_hide(uigtk2, "pc_rb_pipe");
	uigtk2_editable_set_editable(uigtk2, "pc_e_pipe", 0);
	uigtk2_widget_hide(uigtk2, "pc_e_pipe");
#endif
	...
	case ui_tag_print_destination:
		{
			...
			uigtk2_toggle_button_set_active(uigtk2, tb_name, 1);
		}
		break;

	case ui_tag_print_file:
		uigtk2_label_set_text(uigtk2, "pc_l_filename", data ? (const char *)data : "");
		break;
	...
	case ui_tag_print_pipe:
		{
			...
			gtk_entry_buffer_set_text(eb_pipe, uigtk2->printer.pipe, -1);
			uigtk2_widget_set_sensitive(uigtk2, "pc_b_pipe_reset", 0);
			uigtk2_widget_set_sensitive(uigtk2, "pc_b_pipe_apply", 0);
		}
		break;
	...
	case ui_tag_print_count:
		{
			...
			uigtk2_label_set_text(uigtk2, "pc_l_chars", buf);
			uigtk2_widget_set_sensitive(uigtk2, "pc_b_flush", value != 0);
		}
		break;
...
static void pc_set_destination(GtkButton *button, gpointer user_data) {
	...
	for (unsigned i = 0; i < ARRAY_N_ELEMENTS(rb_destinations); i++) {
		GtkButton *dest_button = GTK_BUTTON(gtk_builder_get_object(uigtk2->builder, rb_destinations[i].id));
...
static void pc_pipe_changed(GtkEditable *e, gpointer user_data) {
	(void)e;
	struct ui_gtk2_interface *uigtk2 = user_data;
	uigtk2_widget_set_sensitive(uigtk2, "pc_b_pipe_reset", 1);
	uigtk2_widget_set_sensitive(uigtk2, "pc_b_pipe_apply", 1);
}
